...
// /opt/scoreline/data/league-names.json so they survive deploys. The data
// sources still know the clubs by their original names.

const file = (): string => process.env.LEAGUE_NAMES_FILE ?? path.join(/*turbopackIgnore: true*/ cacheDir(), 'data', 'league-names.json')

let cache: { mtime: number; names: Record<string, string> } = { mtime: -1, names: {} }

/** The changed names and a version that changes with them */
export function leagueNameOverrides(): { version: string; names: Record<string, string> } {
  let mtime = 0
  try {
...
/** Sets a league's name; an empty name goes back to the original */
export function setLeagueName(slug: string, name: string): { error?: string } {
  if (!/^[a-z0-9-]{1,120}$/.test(slug)) return { error: 'Ukendt liga' }
  ...
  const names = { ...leagueNameOverrides().names }
